"""
Step 2 (single-concept): edit each image with FLUX based on the
generated instruction.

For every ``<task_id>.json`` produced by ``instruct_gen.py``, we:

* read the source image (from ``local_image_path`` or, if present and
  object storage is configured, ``storage_image_path``);
* run the FLUX pipeline conditioned on ``detailed_instruction_en``;
* save the result as ``<task_id>_edit.png`` next to the JSON.

Multiple GPUs are used in parallel via ``torch.multiprocessing``. The
script is resume-safe: tasks whose output PNG already exists are
skipped.
"""
...
def get_target_size(width, height, target_area=1024 * 1024):
    """Keep aspect ratio, align both edges to multiples of 16."""
...
def parse_args():
    p = argparse.ArgumentParser(description="Single-pass FLUX image editing")
    p.add_argument("input_dirs", nargs="+", help="One or more directories of edit JSONs")
...
def main():
    ...
    with tqdm(total=total, desc="Flux Editing") as pbar:
        completed = 0
